"""Тесты пространственных признаков защитного контекста (спецификация, разделы 9.3 и 17).

Все проверки — на синтетической геометрии с заранее известным ответом.
Отдельное внимание уделено требованию «не выдумывать координаты»: если вратарь
или соперники не попали в кадр, признаки обязаны быть ``NaN`` или нулём
по явно описанному правилу, а не правдоподобным числом.
"""
...
class TestShotCone:
    ...
    def test_cone_widens_towards_the_goal(self) -> None:
        """У линии ворот конус шире, чем у бьющего: створ 8 ярдов."""
...
class TestOpponentCounts:
    def test_counts_within_radius_inclusive(self) -> None:
        # Соперники на расстоянии 1, 2 и 5 ярдов от точки удара.
