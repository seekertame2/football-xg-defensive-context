...
Проверяются построение URL от commit SHA и раскладка кеша.
Ещё проверяются разбор git-tree и оценка объёма загрузки.
"""
...
class TestProjectDataConfig:
    ...
    def test_selection_is_approved_and_well_formed(self) -> None:
        ...
        После утверждения состав должен быть непустым.
        Каждый элемент должен содержать оба идентификатора.
        """
        config = load_data_config()
