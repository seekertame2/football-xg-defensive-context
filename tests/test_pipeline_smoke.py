"""Smoke-тест полного пайплайна на синтетических событиях.

Проверяется весь путь от событий StatsBomb до метрик и bootstrap.
Он должен проходить целиком и давать ожидаемые таблицы.
Сеть не нужна: события генерируются в схеме StatsBomb.
Поэтому тест выполняется в CI без скачивания данных.

Это проверка связности пайплайна, а не качества модели.
На синтетике осмысленных метрик не бывает.
Поэтому проверяется только структура результатов и соблюдение контрактов.
"""
