"""Лестница моделей M0–M4 (спецификация, раздел 11).

Все модели собираются как `Pipeline` с общим препроцессором, поэтому
предобработка обучается только на train-фолдах, а сравнение наборов признаков
идёт при одинаковой обработке данных.

Дисбаланс классов намеренно **не** компенсируется: ни SMOTE, ни oversampling,
ни ``class_weight="balanced"`` (спецификация, раздел 10.3). Цель — хорошо
откалиброванная вероятность гола, а перевзвешивание классов её смещает.
"""
...
def _dummy(params: dict[str, Any], seed: int) -> DummyClassifier:
    """M0 — предсказывает базовую частоту голов и ничего больше."""
...
def _logistic(params: dict[str, Any], seed: int) -> LogisticRegression:
    """M1/M2/M4 — логистическая регрессия без перевзвешивания классов."""
...
def cross_validated_search(
    ...
    *,
    ...
    Используется `StratifiedGroupKFold`: удары одного матча не разъезжаются
    между фолдами, а доля голов в фолдах сохраняется (спецификация, раздел 10.2).
    Метрика подбора — log loss, главная метрика проекта.
