...
предобработка обучается только на train-фолдах, а наборы признаков
сравниваются при одинаковой обработке данных.

Дисбаланс классов намеренно не компенсируется: ни SMOTE, ни oversampling,
ни ``class_weight="balanced"``. Нужна хорошо откалиброванная вероятность
гола, а перевзвешивание классов её смещает.
"""
...
def cross_validated_search(
    ...
    *,
    ...
    `StratifiedGroupKFold` не разводит удары одного матча по разным фолдам
    и сохраняет долю голов. Метрика подбора это log loss.
