...
На этапах 0–1 здесь реализовано то, что спецификация требует сделать явным
и протестированным до любого моделирования (разделы 8.2, 9.4, 17):

* однозначный маппинг исхода удара в бинарный target;
* фильтр пенальти и ударов серии пенальти;
* проверка, что запрещённые колонки не попали в матрицу признаков.

Построение полного набора признаков добавляется на этапе 2.
"""
...
def map_shot_outcome(outcome_name: str | None) -> int:
    ...
    Неизвестный или отсутствующий исход считается ошибкой данных и приводит к
    исключению: молча превращать его в 0 запрещено спецификацией (раздел 8.2).
...
def is_penalty_shot(shot_type_name: str | None, period: int | None = None) -> bool:
    ...
    Такие удары исключаются из основной выборки: это отдельный
    стандартизированный процесс (спецификация, раздел 8.3).
    """
    ...
    *,
    ...
    if hits:
        raise ForbiddenColumnError(
            ...
            "Эти поля раскрывают исход удара либо идентичность игрока/команды "
            "(спецификация, раздел 9.4)."
        )
