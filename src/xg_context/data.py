"""Избирательная загрузка StatsBomb Open Data с локальным кешем.

Ревизия источника зафиксирована commit SHA, все URL строятся от него.
Репозиторий не клонируется целиком: он весит около 16 ГБ.
Объём любой загрузки известен заранее из git-tree, до скачивания.
"""
...
def download_all_metadata(
    ...
    Метаданные малы, поэтому берутся целиком.
    Это даёт полную перепись соревнований, сезонов и матчей без догадок.
    """
