"""Предобработка признаков внутри scikit-learn Pipeline.

Все преобразования, включая imputer и масштабирование, живут внутри `Pipeline`.
Поэтому они обучаются только на train и никогда не видят target валидации или теста.

Если вратарь не попал в кадр, его координаты неизвестны.
Придумывать их нельзя, поэтому берётся медиана train плюс флаг ``has_goalkeeper``.
Флаг позволяет модели отличить "вратарь близко" от "вратаря не видно".
"""
...
def build_preprocessor(
    ...
    *,
    ...
    features:
        ...
        Набор проверяется на запрещённые поля до сборки.
        Пайплайн обязан падать, если в матрицу попало поле из `FORBIDDEN_FEATURE_COLUMNS`.
    scale_numeric:
        Масштабировать числовые признаки.
        Нужно логистической регрессии, бесполезно деревьям - для них можно отключить.
