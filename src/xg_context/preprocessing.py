...
Все преобразования, включая imputer и масштабирование, живут внутри
`Pipeline`. Поэтому они обучаются только на train и никогда не видят
target валидации или теста.

Если вратарь не попал в кадр, его координаты неизвестны. Придумывать их
нельзя, поэтому берётся медиана train плюс флаг ``has_goalkeeper``.
