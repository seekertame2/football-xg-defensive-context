...
Все функции чистые и векторизованные по NumPy.
Поэтому их легко покрыть тестами и переиспользовать в ноутбуках.

Система координат: поле 120 x 80.
...
def goal_mouth_angle(x: ArrayLike, y: ArrayLike, *, degrees: bool = False) -> NDArray[np.float64]:
    ...
    Это угол между направлениями на две штанги.
    Он считается через ``arctan2`` от векторного и скалярного произведения.
    Такая форма устойчива и на острых углах, и почти на линии ворот.
    Разность арктангенсов и ``arccos`` в этих случаях теряют точность.

    Результат лежит в ``[0, pi]`` радиан или в градусах при ``degrees=True``.
    На линии ворот между штангами угол равен ровно ``pi``.
    Вне отрезка ворот на той же линии он равен ``0``.
    ...
    # Вырожденный случай: точка совпадает со штангой - вектор нулевой, arctan2(0, 0) вернул бы 0.
    # Такие удары геометрически неинформативны.
...
# Функции ниже работают с координатами соперников одного удара.
# `freeze_frame` показывает только игроков, попавших в кадр.
# Поэтому любой счётчик здесь означает "сколько соперников видно".
...
def point_in_shot_cone(
    ...
    Это и есть "конус удара".
    Соперник внутри него перекрывает часть створа, видимого бьющему.

    Проверка - через знаки векторных произведений для трёх рёбер треугольника.
    Форма устойчива к ориентации треугольника и не требует деления.
    Бьющий может быть и слева, и справа от центра.

    Вырожденный случай это удар с самой линии ворот.
    Треугольник схлопывается, конус пуст, и функция возвращает ``False``.
    """
...
def count_opponents_within(
    ...
    """Сколько видимых соперников находится не дальше ``radius`` от бьющего.

    Радиус задаётся в единицах координат StatsBomb.
    """
    distances = point_distance(shot_x, shot_y, opponent_x, opponent_y)
...
def opponents_goal_side(
    ...
    Определение простое и интерпретируемое: соперник ближе к воротам по оси ``x``, чем бьющий.
    В отличие от конуса удара, попадание в створ здесь не требуется.
    Величина описывает общую плотность обороны перед мячом.
    """
    px = _as_float_array(opponent_x)
...
def defensive_context(
    ...
    Notes
    -----
    Все счётчики считают только видимых в кадре игроков.
    Отсутствие вратаря даёт ``NaN`` во всех вратарских признаках.
    Флаг ``has_goalkeeper`` при этом равен ``0``, а координаты не выдумываются.
    """
    px = _as_float_array(opponent_x)
    py = _as_float_array(opponent_y)
