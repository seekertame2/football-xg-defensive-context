...
Удары одного матча никогда не попадают в разные части выборки. Случайное
разбиение строк дало бы утечку: удары одного эпизода похожи между собой.

Метод это жадное балансирующее назначение матчей. Внутри каждой лиги матчи
обрабатываются в детерминированном порядке от богатых голами к бедным,
и каждый матч уходит в ту часть, которой сильнее всего не хватает ударов
относительно её квоты. Так выравниваются и объём, и доля голов, чего
не даёт ни случайное разбиение матчей, ни `GroupShuffleSplit`.
"""
...
def save_split(split: SplitResult, path: str | Path, shot_ids: dict[str, list[str]]) -> None:
    ...
    Тестовые идентификаторы фиксируются один раз. Сохранённый список позволяет
    проверить, что test не использовался для выбора признаков.
    """
