"""Разбиение выборки с группировкой по матчам (этап 3 спецификации, раздел 10.1).

Ключевые требования, которые здесь реализованы:

* удары одного матча **никогда** не попадают в разные части выборки;
* наивное случайное разбиение строк запрещено;
* разбиение детерминировано при фиксированном seed;
* каждая часть содержит сопоставимые доли четырёх лиг и сопоставимую долю голов
  (решение владельца проекта от 2026-08-23);
* тестовая выборка фиксируется один раз и сохраняется на диск.

Метод — жадное балансирующее назначение матчей. Внутри каждой лиги матчи
обрабатываются в детерминированном порядке от «богатых голами» к «бедным»,
и каждый матч отправляется в ту часть, которой сильнее всего не хватает
ударов относительно её квоты. Такой порядок выравнивает одновременно и объём,
и долю голов, чего не гарантирует ни случайное разбиение матчей, ни
`GroupShuffleSplit`.
"""
...
def make_grouped_split(
    ...
    *,
    ...
    ----------
    shots:
        Таблица ударов; нужны колонки ``group_column``, ``target_column``
        и, при балансировке, ``stratify_column``.
    stratify_column:
        Колонка, доли значений которой должны совпадать во всех частях
        (у нас — лига). Балансировка выполняется независимо внутри каждого
        значения, поэтому доли сохраняются по построению.
...
def _assign_stratum(
    ...
    # Детерминированный порядок: сначала перемешиваем с seed, зависящим от лиги,
    # затем сортируем по числу голов — так матчи с одинаковым числом голов
    # попадают в разные части, а не подряд в одну.
    ...
    assignment: dict[int, str] = {}
    for row in order:
        # Часть с наибольшим относительным дефицитом ударов; при равенстве —
        # с наименьшей текущей долей голов, чтобы выровнять и target.
        candidate = min(
            sizes,
            key=lambda name: (
...
def save_split(split: SplitResult, path: str | Path, shot_ids: dict[str, list[str]]) -> None:
    ...
    Тестовые идентификаторы фиксируются один раз: спецификация запрещает
    использовать test для выбора признаков или гиперпараметров, и сохранённый
    список позволяет это проверить.
    """
