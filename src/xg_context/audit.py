...
Считает, сколько есть матчей и ударов, у какой доли ударов есть корректный
``shot.freeze_frame``, как часто распознаётся вратарь и чем удары с контекстом
отличаются от остальных. Модельный датасет здесь не строится.
"""
...
def selection_bias_table(shots: pd.DataFrame) -> pd.DataFrame:
    ...
    Большие расхождения означают, что ``context_eligible_shots`` не случайное
    подмножество, и это нужно оговорить в выводах.
    """
...
def probe_three_sixty(
    ...
    Нужно, чтобы понять, даёт ли отдельный источник 360 достаточно новой
    информации для существенно большего объёма загрузки.
    """
