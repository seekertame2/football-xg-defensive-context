"""Аудит доступности данных StatsBomb Open Data.

Считает, сколько есть матчей и ударов.
Считает долю ударов с корректным ``shot.freeze_frame``.
Проверяет, как часто распознаётся вратарь.
Сравнивает удары с контекстом и без него.
Модельный датасет здесь не строится.
"""
...
def build_coverage_table(
    ...
    Основана на полной переписи метаданных и git-tree источника.
    Поэтому числа матчей и файлов точны, а не оценены по выборке.
    """
...
def sample_matches_for_audit(
    ...
    *,
    ...
    Выборка стратифицирована по competition-season.
    Из каждого сезона берётся до ``matches_per_season`` матчей.
    Это даёт покрытие всех эпох и схем, не скачивая 12.8 ГБ событий целиком.
    """
...
def audit_match_shots(
    ...
    *,
    ...
    их доля тоже входит в отчёт.
    Фильтрация выполняется на уровне анализа.
    """
...
def shots_to_frame(records: Iterable[Mapping[str, Any]]) -> pd.DataFrame:
    ...
    # Удар пригоден для defensive-context модели, если есть freeze_frame.
    # В кадре при этом нужен хотя бы один соперник с координатами.
    frame["has_context"] = (
        frame["is_eligible"]
...
def selection_bias_table(shots: pd.DataFrame) -> pd.DataFrame:
    ...
    Большие расхождения означают, что ``context_eligible_shots`` не случайно.
    Такое смещение нужно оговорить в выводах.
    """
...
def probe_three_sixty(
    ...
    Нужно, чтобы понять, даёт ли источник 360 достаточно новой информации.
    Он стоит существенно большего объёма загрузки.
    """
