"""Вероятностные метрики, калибровка и оценка неопределённости (раздел 12).

Главные метрики проекта — log loss и калибровка: xG оценивается как
вероятностная модель, а не как классификатор. ROC-AUC и PR-AUC приводятся
дополнительно и не заменяют их.
